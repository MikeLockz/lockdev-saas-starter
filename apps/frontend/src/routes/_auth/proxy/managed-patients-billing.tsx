...
interface ManagedPatientSubscription {
  patient_id: string;
  patient_name: string;
    ...
    <div className="container mx-auto p-6">
      ...
      {subscriptions && subscriptions.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Patients can assign you as their billing manager from their
              billing page.
            </p>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4">
          {subscriptions?.map((sub: ManagedPatientSubscription) => (
            <Card key={sub.patient_id}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button asChild>
                    <Link
                      to="/proxy/managed-patients/$managedPatientId/billing"
                      params={{ managedPatientId: sub.patient_id }}
                    >
                      Manage Billing
                    </Link>
                  </Button>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">
                      {sub.plan_id || "No active plan"}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">MRR</p>
                    <p className="font-medium">
                      {formatCurrency(sub.mrr_cents / 100)}
                    </p>
                  </div>
