...
export const useAssignProxy = (patientId: string) => {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["patient-proxies", currentOrgId, patientId] });
        },
    });
};
...
export const useUpdateProxyPermissions = (patientId: string) => {
    ...
    return useMutation({
        mutationFn: async ({ assignmentId, data }: { assignmentId: string; data: ProxyAssignmentUpdate }) => {
            if (!currentOrgId) throw new Error("No organization selected");
