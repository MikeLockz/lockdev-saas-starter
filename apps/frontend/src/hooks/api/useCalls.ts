  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<Call[]>(
        `/api/v1/organizations/${currentOrgId}/calls?${params.toString()}`,
      );
  ...
  return useMutation({
    mutationFn: async (data: CreateCallData) => {
      ...
      const response = await api.post<Call>(
        `/api/v1/organizations/${currentOrgId}/calls`,
        data,
      );
      return response.data;
    },
...
export const useUpdateCall = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: UpdateCallData }) => {
      if (!currentOrgId) throw new Error("No organization selected");
      const response = await api.patch<Call>(
        `/api/v1/organizations/${currentOrgId}/calls/${id}`,
        data,
      );
      return response.data;
    },
