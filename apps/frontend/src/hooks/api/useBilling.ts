  ...
  return useQuery<SubscriptionStatus>({
    ...
    queryFn: async () => {
      ...
      const response = await api.get(
        `/api/v1/organizations/${currentOrgId}/billing/subscription`,
      );
  ...
  return useMutation<CheckoutSessionResponse, Error, CheckoutSessionRequest>({
    mutationFn: async (data) => {
      ...
      const response = await api.post(
        `/api/v1/organizations/${currentOrgId}/billing/checkout`,
        data,
      );
      return response.data;
    },
...
export const useBillingPortal = () => {
  ...
  return useMutation<PortalSessionResponse, Error, void>({
    mutationFn: async () => {
      if (!currentOrgId) throw new Error("No organization selected");
      const response = await api.post(
        `/api/v1/organizations/${currentOrgId}/billing/portal`,
      );
      return response.data;
    },
    onSuccess: (data) => {
