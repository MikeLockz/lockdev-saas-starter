...
export function useSuperAdminOrganization(orgId: string) {
  return useQuery<OrganizationAdmin>({
    ...
    queryFn: async () => {
      const response = await api.get(
        `/api/v1/admin/super-admin/organizations/${orgId}`,
      );
      return response.data;
    },
    enabled: !!orgId,
...
export function useUpdateOrganization() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      orgId: string;
      data: { is_active?: boolean; subscription_status?: string };
    }) => {
...
export function useSuperAdminUsers(params: UserSearchParams = {}) {
  return useQuery<UserListResponse>({
    ...
    queryFn: async () => {
      const response = await api.get("/api/v1/admin/super-admin/users", {
        params,
      });
      return response.data;
    },
...
export function useUnlockUser() {
  ...
  return useMutation({
    mutationFn: async (userId: string) => {
      const response = await api.patch(
        `/api/v1/admin/super-admin/users/${userId}/unlock`,
      );
      return response.data;
    },
    onSuccess: () => {
...
export function useUpdateUserAdmin() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      userId: string;
      data: { is_super_admin?: boolean };
    }) => {
      const response = await api.patch(
        `/api/v1/admin/super-admin/users/${userId}`,
        data,
      );
