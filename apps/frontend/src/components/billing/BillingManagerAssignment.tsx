...
import {
  useAssignBillingManager,
  useRemoveBillingManager,
...
export function BillingManagerAssignment({
  patientId,
  currentBillingManagerId,
}: Props) {
  ...
  const [selectedProxyId, setSelectedProxyId] = useState<string | undefined>(
    currentBillingManagerId || undefined,
  );

  const activeProxies = proxies?.filter((p) => p.status === "ACTIVE") || [];
  const currentManager = activeProxies.find(
    (p) => p.proxy_user_id === currentBillingManagerId,
  );
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Assign a proxy to manage billing on your behalf. They will receive all
          billing emails and can manage your subscription.
        </CardDescription>
      </CardHeader>
        ...
        {currentManager ? (
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {currentManager.proxy_name} will no longer be able to
                      manage your billing. You will receive all billing emails
                      directly. You can reassign them later if needed.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
              ...
              >
                ...
                <SelectContent>
                  {activeProxies.map((proxy) => (
                    <SelectItem
                      key={proxy.proxy_user_id}
                      value={proxy.proxy_user_id}
                    >
          ...
          <div className="text-center py-4">
            <p className="text-muted-foreground text-sm">
              You don't have any active proxies. Add a proxy first to assign
              them as billing manager.
            </p>
          </div>
        )}
      </CardContent>
