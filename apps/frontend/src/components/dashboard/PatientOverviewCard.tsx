import { Calendar, Clock, Stethoscope, User, Users } from "lucide-react";
...
import { type CareTeamMember, useCareTeam } from "@/hooks/useCareTeam";
...
export function PatientOverviewCard({ patientId }: PatientOverviewCardProps) {
  const { data: appointments, isLoading: isApptLoading } =
    usePatientUpcomingAppointments(patientId);
  const { data: careTeam, isLoading: isCareTeamLoading } =
    useCareTeam(patientId);
    ...
    return (
      <Card className="lg:col-span-4">
        <CardHeader>
          ...
          <CardDescription>
            Loading your appointments and care team...
          </CardDescription>
        </CardHeader>
