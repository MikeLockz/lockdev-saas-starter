import { Calendar, Eye, FileText, MessageSquare, Users } from "lucide-react";
...
import {
  type ProxyPatient,
  useMyProxyPatients,
} from "@/hooks/api/useMyProxyPatients";
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-medium truncate">
                      {proxyPatient.patient.first_name}{" "}
                      {proxyPatient.patient.last_name}
                    </p>
                    <Badge variant="secondary" className="text-xs">
                      {relationshipLabels[proxyPatient.relationship_type] ||
                        proxyPatient.relationship_type}
                    </Badge>
                  </div>
                  <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                    <span>
                      DOB:{" "}
                      {new Date(proxyPatient.patient.dob).toLocaleDateString()}
                    </span>
                    {proxyPatient.patient.medical_record_number && (
                      <span>
                        MRN: {proxyPatient.patient.medical_record_number}
                      </span>
                    )}
                  </div>
