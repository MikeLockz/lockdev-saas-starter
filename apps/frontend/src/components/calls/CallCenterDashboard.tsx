    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(val) =>
              setValue("direction", val as "INBOUND" | "OUTBOUND")
            }
            disabled={!!activeCall}
          >
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(val) =>
              setValue(
                "status",
                val as "QUEUED" | "IN_PROGRESS" | "COMPLETED" | "MISSED",
              )
            }
          >
            <SelectTrigger>
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(val) =>
            setValue("patient_id", val === "none" ? "" : val)
          }
          disabled={!!activeCall}
        >
          <SelectTrigger>
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-4">
        <div
          className={`p-2 rounded-full ${call.direction === "INBOUND" ? "bg-blue-100 text-blue-600" : "bg-green-100 text-green-600"}`}
        >
...
export function CallCenterDashboard() {
  ...
  const activeCalls =
    calls?.filter((c: Call) => c.status === "IN_PROGRESS") || [];
  const historyCalls =
    calls?.filter(
      (c: Call) => c.status !== "IN_PROGRESS" && c.status !== "QUEUED",
    ) || [];
        ...
        <div className="flex justify-between items-center">
          ...
          <Dialog open={isLogOpen} onOpenChange={setIsLogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>
                  {selectedCall ? "Edit Call Details" : "Log New Call"}
                </DialogTitle>
              </DialogHeader>
              <CallLogForm
                activeCall={selectedCall || undefined}
