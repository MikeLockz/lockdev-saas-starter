...
import { usePatientProxies, useRevokeProxy, type ProxyAssignment } from "@/hooks/api/usePatientProxies";
...
export function ProxyList({ patientId, patientName }: ProxyListProps) {
    ...
    return (
        <div className="space-y-4">
                                ...
                                <TableRow key={proxy.id}>
                                    ...
                                    <TableCell>
                                        <Badge variant="outline">
                                            {RELATIONSHIP_LABELS[proxy.relationship_type] || proxy.relationship_type}
                                        </Badge>
                                    </TableCell>
                                    <TableCell>
            ...
            <AlertDialog open={!!revokeProxy} onOpenChange={(open) => !open && setRevokeProxy(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Revoke Proxy Access?</AlertDialogTitle>
                        <AlertDialogDescription>
                            This will immediately revoke access for{" "}
                            <strong>{revokeProxy?.user.display_name || revokeProxy?.user.email}</strong>.
                            They will no longer be able to view or manage this patient's records.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
