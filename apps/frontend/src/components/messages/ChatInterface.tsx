...
export function ChatInterface({ threadId }: ChatInterfaceProps) {
  ...
  }, [threadId, thread?.unread_count, markRead.mutate]);
    ...
    <div className="flex flex-col h-full bg-background rounded-lg border shadow-sm overflow-hidden">
      ...
      <div className="p-4 border-b flex justify-between items-center bg-card">
        <div>
          ...
          <p className="text-xs text-muted-foreground">{otherParticipants}</p>
        </div>
      </div>
        ...
        {thread.messages?.map((msg) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={cn("space-y-1", isMe ? "items-end" : "items-start")}
              >
                <div
                  className={cn(
