...
describe('Axios Interceptors', () => {
    ...
    it('should add Authorization header when user is logged in', async () => {
        ...
        expect(mock.history.get[0].headers).toHaveProperty('Authorization', 'Bearer mock-token');
    });
    ...
    it('should add X-Organization-Id header when org is selected', async () => {
        ...
        expect(mock.history.get[0].headers).toHaveProperty('X-Organization-Id', 'org-123');
    });

    it('scrolls through allowed domains (sanity check on interceptor logic)', async () => {
        // No auth, no org
        (auth as any).currentUser = null;
        vi.mocked(useOrgStore.getState).mockReturnValue({ currentOrgId: null } as any);
