...
async def verify_latest_consents(user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> User:
    """
    ...
    user_signatures_result = await db.execute(
        select(UserConsent).where(UserConsent.user_id == user.id).where(UserConsent.document_id.in_(active_doc_ids))
    )
