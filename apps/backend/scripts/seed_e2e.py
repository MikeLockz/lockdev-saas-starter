...
async def seed():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # 1. ORGANIZATION
        # ============================================================
        org = Organization(name="Lockdev Clinic", tax_id="12-3456789", subscription_status="ACTIVE", is_active=True)
        session.add(org)
        await session.flush()
        ...
        await session.flush()
        print("  ✓ Users: 6 created")

        # ============================================================
        # 3. PROFILES
        # ============================================================
        # Staff Profile
        staff_profile = Staff(user_id=staff_user.id, organization_id=org.id, job_title="NURSE", employee_id="ST-001")
        ...
        # Proxy Profile
        proxy_profile = Proxy(user_id=proxy_user.id, relationship_to_patient="PARENT")
        session.add(proxy_profile)
        await session.flush()
        print("  ✓ Profiles: Staff, 2 Providers, 2 Patients, Proxy")

        # ============================================================
        # 4. ORGANIZATION MEMBERSHIPS
        # ============================================================
        # Staff Member
        staff_member = OrganizationMember(user_id=staff_user.id, organization_id=org.id, role="STAFF")
        session.add(staff_member)

        # Provider Members
        provider_member = OrganizationMember(user_id=provider_user.id, organization_id=org.id, role="PROVIDER")
        session.add(provider_member)

        provider_member2 = OrganizationMember(user_id=provider_user2.id, organization_id=org.id, role="PROVIDER")
        session.add(provider_member2)

        # Patient Member (so patient user can access org data in dashboard)
        patient_member = OrganizationMember(user_id=patient_user.id, organization_id=org.id, role="PATIENT")
        session.add(patient_member)

        # Proxy Member (so proxy user can access org data in dashboard)
        proxy_member = OrganizationMember(user_id=proxy_user.id, organization_id=org.id, role="PROXY")
        session.add(proxy_member)

        # Patient in Org
        org_patient = OrganizationPatient(organization_id=org.id, patient_id=patient_profile.id, status="ACTIVE")
        session.add(org_patient)

        # Second Patient in Org (minor child)
        org_patient2 = OrganizationPatient(organization_id=org.id, patient_id=patient_profile2.id, status="ACTIVE")
        session.add(org_patient2)
        print("  ✓ Organization memberships created")
        ...
        print("  ✓ Proxy assignments created (2 patients)")
        ...
        print("  ✓ Care team: 2 assignments (PRIMARY, SPECIALIST)")
        ...
        calls_data = [
            ...
            },
            {
                ...
                "ended_at": now() - timedelta(hours=4) + timedelta(minutes=8),
            ...
            },
            {
                ...
                "ended_at": now() - timedelta(hours=8) + timedelta(seconds=30),
                "duration_seconds": 30,
            },
        ]
        for i, c in enumerate(calls_data):
            ...
            session.add(call)
        print("  ✓ Calls: 6 created (QUEUED, IN_PROGRESS, COMPLETED×3, MISSED)")
        ...
        staff_tasks_data = [
            {
                ...
                "title": "Urgent: Follow up on lab results",
            },
            {
                "status": "TODO",
                "priority": "HIGH",
                "due_date": today,
                "title": "Call patient about prescription refill",
            },
            {"status": "IN_PROGRESS", "priority": "HIGH", "due_date": today, "title": "Insurance pre-authorization"},
            {
                ...
                "title": "Update patient contact info",
        ...
        provider_tasks_data = [
            {
                ...
                "priority": "URGENT",
                "due_date": today,
                "title": "Review CT scan results - Patient Smith",
            },
            {"status": "TODO", "priority": "HIGH", "due_date": today, "title": "Sign off on discharge summary"},
            {
                ...
                "priority": "MEDIUM",
                "due_date": today + timedelta(days=1),
                "title": "Complete referral letter for cardiology",
            },
            {
                "status": "IN_PROGRESS",
                "priority": "HIGH",
                "due_date": today,
                "title": "Medication reconciliation for Test Patient",
            },
            {
                "status": "DONE",
                "priority": "MEDIUM",
        ...
        for t in provider_tasks_data:
            ...
            session.add(task)
        print("  ✓ Tasks: 9 created (4 for staff, 5 for provider)")
        ...
        appts_data = [
            ...
            },
            {
                ...
                "notes": "Patient in good health. Follow up in 2 weeks.",
            },
            {
                "status": "CANCELLED",
                "type": "FOLLOW_UP",
        ...
        for a in appts_data:
            appt_time = now().replace(hour=a["hour"], minute=0, second=0, microsecond=0) + timedelta(
                days=a["offset_days"]
            )
            appt = Appointment(
                organization_id=org.id,
        ...
        today_appts = [
            {
                ...
                "hour": 9,
                "minute": 0,
                "reason": "Post-op check - healing well",
            ...
            },
            {
                ...
                "hour": 14,
                "minute": 0,
                "reason": "Acute back pain evaluation",
        ...
        for ta in today_appts:
            ...
            session.add(appt)
        print("  ✓ Appointments: 10 created (5 past/future, 5 today for Provider Dashboard)")
        ...
        docs_data = [
            ...
            },
            {
                "status": "CONFIRMED",
                "doc_type": "LAB_RESULT",
                "file_name": "cholesterol_panel.pdf",
                "file_type": "application/pdf",
                "size": 85000,
            },
            {
            ...
            },
            {
                ...
                "file_name": "hipaa_consent_signed.pdf",
                "file_type": "application/pdf",
                "size": 45000,
            },
            {
        ...
        for i, d in enumerate(docs_data):
            ...
            session.add(doc)
        print("  ✓ Documents: 5 created (PENDING×1, CONFIRMED×4)")
        ...
        notifications_data = [
            ...
            },
            {
                ...
                "type": "BILLING",
                "title": "Invoice Ready",
                "body": "Your monthly invoice is ready for review",
                "is_read": True,
            ...
            # Provider notifications
            {
                ...
                "body": "Patient Test Patient has requested an appointment",
                "is_read": False,
            },
            {
                "user": provider_user,
                ...
                "body": "You have a new message from Test Patient",
                "is_read": False,
            },
            {
                "user": provider_user,
                ...
                "body": "Tomorrow's schedule has been updated",
                "is_read": False,
            },
            {
                "user": provider_user,
                ...
                "body": "You have 3 appointments today",
                "is_read": True,
            ...
            },
            {
                ...
                "body": "Dr. Smith is requesting a cardiology consult",
                "is_read": False,
            },
            {
                "user": provider_user2,
        ...
        for n in notifications_data:
            ...
            session.add(notif)
        print("  ✓ Notifications: 12 created (9 unread, 3 read)")
        ...
        thread1 = MessageThread(organization_id=org.id, subject="Lab Results Discussion", patient_id=patient_profile.id)
        session.add(thread1)
        await session.flush()
        ...
        session.add(
            MessageParticipant(thread_id=thread1.id, user_id=patient_user.id, last_read_at=now() - timedelta(days=1))
        )
        ...
        session.add(
            Message(
                ...
                body="Hi, I've reviewed your lab results. Everything looks good overall.",
            )
        )
        await session.flush()
        session.add(
            Message(
                ...
                body="Thank you, doctor. Should I schedule a follow-up?",
            )
        )
        await session.flush()
        session.add(
            Message(
                thread_id=thread1.id,
                sender_id=provider_user.id,
                body="Yes, please schedule one in 3 months. We'll recheck your cholesterol.",
            )
        )

        # Thread 2: Staff-Patient (Resolved, fully read)
        thread2 = MessageThread(
            organization_id=org.id, subject="Appointment Rescheduling", patient_id=patient_profile.id
        )
        session.add(thread2)
        await session.flush()
        ...
        session.add(
            Message(
                ...
                body="Hi, we need to reschedule your upcoming appointment. What times work for you?",
            )
        )
        await session.flush()
        session.add(
            Message(
                thread_id=thread2.id,
                sender_id=patient_user.id,
                body="Tuesday or Wednesday afternoon would work best. Thank you!",
            )
        )
        print("  ✓ Message threads: 2 created with 5 messages total")
        ...
        session.add(
            SupportMessage(
                ...
                sender_id=patient_user.id,
                body="I'm getting an error when trying to view my lab results. The page just shows a loading spinner.",
            )
        )
        ...
        session.add(
            SupportMessage(
                ...
                sender_id=patient_user.id,
                body="I received a bill for my visit last week but my insurance should have covered it.",
            )
        )
        await session.flush()
        session.add(
            SupportMessage(
                ticket_id=ticket2.id,
                sender_id=staff_user.id,
                body="I'm looking into this for you. Can you provide your insurance policy number?",
            )
        )
        await session.flush()
        ...
        session.add(
            SupportMessage(
                ticket_id=ticket3.id, sender_id=proxy_user.id, body="I need to update my email address in the system."
            )
        )
        await session.flush()
        session.add(
            SupportMessage(
                ...
                sender_id=staff_user.id,
                body="Done! I've updated your email. Please verify you receive the confirmation.",
                is_internal=False,
            )
        )
        await session.flush()
        session.add(
            SupportMessage(
                ticket_id=ticket3.id,
                sender_id=staff_user.id,
                body="[Internal] Verified email change in admin panel.",
                is_internal=True,
            )
        )
        ...
        session.add(
            SupportMessage(
                ...
                sender_id=patient_user.id,
                body="Just wanted to say the new appointment booking feature is great!",
            )
        )
        print("  ✓ Support tickets: 4 created with 8 messages (OPEN, IN_PROGRESS, RESOLVED, CLOSED)")
        ...
        tos_current = ConsentDocument(
            doc_type="TOS", version="1.0", content_text="Terms of Service v1.0...", is_active=True
        )
        hipaa_current = ConsentDocument(
            doc_type="HIPAA", version="1.0", content_text="HIPAA Notice v1.0...", is_active=True
        )
        privacy_current = ConsentDocument(
            doc_type="PRIVACY", version="1.0", content_text="Privacy Policy v1.0...", is_active=True
        )
        # Old inactive version
        tos_old = ConsentDocument(
            doc_type="TOS", version="0.9", content_text="Terms of Service v0.9...", is_active=False
        )
        ...
        for doc in [tos_current, hipaa_current, privacy_current]:
            ...
            session.add(consent)
        print("  ✓ Consent documents: 4 created, 3 signed by E2E user")
        ...
        print("  ✓ Invitations: 3 created (PENDING valid, PENDING expired, ACCEPTED)")
