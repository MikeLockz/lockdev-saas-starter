...
def parse_epic(file_path: Path) -> dict:
    ...
    return {"title": title, "description": description, "path": file_path}
...
def parse_story(file_path: Path) -> dict:
    ...
    return {"title": title, "description": description, "is_done": is_done}
...
async def main():
    ...
    if existing_project:
        ...
        """
        p_res = await linear_query(
            create_project_mutation, {"input": {"name": project_name, "teamIds": [team["id"]], "state": "planned"}}
        )
    ...
    for epic_dir in epic_dirs:
        ...
        for story_file in story_files:
            ...
            if story_data["is_done"]:
                print("     -> Note: Story is marked DONE in docs. Please update status in Linear manually.")
