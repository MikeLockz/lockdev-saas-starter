...
def upgrade() -> None:
    ...
    op.create_foreign_key("fk_providers_organization", "providers", "organizations", ["organization_id"], ["id"])
    ...
    # Add foreign key for organization_id
    op.create_foreign_key("fk_staff_organization", "staff", "organizations", ["organization_id"], ["id"])
    ...
    op.create_index("ix_care_team_org_patient", "care_team_assignments", ["organization_id", "patient_id"])

    op.create_index(
        "ix_care_team_active", "care_team_assignments", ["patient_id"], postgresql_where=sa.text("removed_at IS NULL")
    )
