...
def upgrade() -> None:
    ...
    patient_columns = [c["name"] for c in insp.get_columns("patients")]

    # Add billing_manager fields to patients table
    if "billing_manager_id" not in patient_columns:
    ...
    if "billing_manager_assigned_at" not in patient_columns:
        op.add_column(
            "patients",
            sa.Column("billing_manager_assigned_at", sa.DateTime(timezone=True), nullable=True),
        )

    if "billing_manager_assigned_by" not in patient_columns:
    ...
    if "subscription_overrides" not in existing_tables:
        op.create_table(
