...
async def test_superuser(db_session):
    ...
    user = User(email=email, password_hash="dummy_hash", display_name="Audit Admin", is_super_admin=True)
    db_session.add(user)
...
async def test_list_audit_logs_filtering(client: AsyncClient, superuser_token_headers: dict):
    ...
    response = await client.get(
        "/api/v1/admin/audit-logs", headers=superuser_token_headers, params={"action_type": "READ"}
    )
    assert response.status_code == 200
    data = response.json()
