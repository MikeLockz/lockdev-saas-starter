...
async def test_stripe_webhook_checkout_completed(db_session):
    ...
    org = Organization(name="Test Org Billing", stripe_customer_id=stripe_customer_id, subscription_status="INCOMPLETE")
    db_session.add(org)
    ...
    mock_event = MockStripeEvent(
        type="checkout.session.completed", data={"customer": stripe_customer_id, "subscription": "sub_123"}
    )
...
async def test_stripe_webhook_payment_failed(db_session):
    ...
    stripe_customer_id = f"cus_{uuid.uuid4()}"
    org = Organization(
        name="Test Org Failed Payment", stripe_customer_id=stripe_customer_id, subscription_status="ACTIVE"
    )
    db_session.add(org)
    await db_session.commit()

    mock_event = MockStripeEvent(
        type="invoice.payment_failed", data={"customer": stripe_customer_id, "attempt_count": 1}
    )

    with patch("stripe.Webhook.construct_event", return_value=mock_event):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://localhost") as client:
            response = await client.post("/api/v1/webhooks/stripe", json={}, headers={"Stripe-Signature": "fake"})
...
async def test_stripe_webhook_invalid_signature():
    ...
    with patch("stripe.Webhook.construct_event", side_effect=ValueError("Invalid signature")):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://localhost") as client:
            response = await client.post("/api/v1/webhooks/stripe", json={}, headers={"Stripe-Signature": "bad_sig"})
