...
from src.models import Organization, OrganizationMember, Provider, User
...
class TestCareTeamAPI:
    ...
    @pytest.mark.asyncio
    async def test_assign_provider_to_care_team(self, authenticated_client, test_org, test_patient, test_provider):
        """Test assigning a provider to a patient's care team."""
        data = {"provider_id": str(test_provider.id), "role": "PRIMARY"}
        response = await authenticated_client.post(
            f"/api/v1/organizations/{test_org.id}/patients/{test_patient.id}/care-team", json=data
        )
    ...
    @pytest.mark.asyncio
    async def test_get_care_team(self, authenticated_client, test_org, test_patient, test_provider):
    ...
    @pytest.mark.asyncio
    async def test_remove_from_care_team(self, authenticated_client, test_org, test_patient, test_provider):
    ...
    @pytest.mark.asyncio
    async def test_duplicate_assignment_rejected(self, authenticated_client, test_org, test_patient, test_provider):
