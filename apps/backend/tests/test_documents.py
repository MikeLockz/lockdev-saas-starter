...
async def test_generate_upload_url_api_error():
    ...
    mock_s3.generate_presigned_post = Mock(side_effect=ClientError(error_response, "GeneratePresignedPost"))
...
async def test_download_from_s3_success():
    ...
    mock_response = {"Body": Mock(read=Mock(return_value=test_content))}
...
async def test_extract_text_api_error():
    ...
    mock_textract.detect_document_text = Mock(side_effect=ClientError(error_response, "DetectDocumentText"))
