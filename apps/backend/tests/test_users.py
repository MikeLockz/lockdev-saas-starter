...
async def test_sessions(db_session, test_user):
    ...
    for i, ua in enumerate(
        ...
        session = UserSession(
            ...
            last_active_at=datetime.now(UTC) - timedelta(minutes=i * 10),
        )
...
class TestAuthFlow:
    ...
    async def test_get_or_create_session_creates_new(self, db_session, test_user):
        ...
        stmt = select(UserSession).where(
            UserSession.user_id == test_user.id, UserSession.firebase_uid == "new_firebase_uid"
        )
...
class TestDataExport:
    ...
    async def test_export_request_placeholder(self, db_session, test_user):
        ...
        assert estimated_completion > datetime.now(UTC)
