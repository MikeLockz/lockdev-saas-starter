...
// Hooks
export function usePatientDocuments(patientId: string | undefined, documentType?: string) {
    const currentOrgId = useOrgStore((state) => state.currentOrgId);

    return useQuery<PaginatedDocuments>({
        queryKey: ['documents', currentOrgId, patientId, documentType],
        queryFn: async () => {
            if (!currentOrgId || !patientId) throw new Error('Missing org or patient');
...
export function useUploadDocument(patientId: string) {
    ...
    return useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['documents', currentOrgId, patientId] });
        },
    });
}
