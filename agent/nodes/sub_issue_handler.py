    ...
    if request_type == "requires_contract":
        ...
        if schema:
            ...
            for field, details in schema.items():
                if isinstance(details, dict):
                    ...
                    sections.append(
                        f"- `{field}` ({field_type}, {required}): {validation}\n"
                    )
        ...
        if resources:
            ...
            for res in resources:
                sections.append(
                    f"- **{res.get('name', 'unknown')}** ({res.get('type', 'unknown')}): {res.get('description', '')}\n"
                )
        ...
        if env_vars:
            ...
            for var in env_vars:
                ...
                sections.append(
                    f"- `{var.get('name', 'VAR')}` ({req}): {var.get('description', '')}\n"
                )
        ...
        if components:
            ...
            for comp in components:
                sections.append(
                    f"### `{comp.get('path', comp.get('name', 'unknown'))}`\n"
                )
        ...
        if api_contracts:
            ...
            for api in api_contracts:
                sections.append(
                    f"### `{api.get('method', 'GET')} {api.get('path', '/unknown')}`\n"
                )
...
def sub_issue_handler_node(state: AgentState) -> dict:
    ...
    if not tech_spec or not issue:
        return {
            ...
            + ["No technical spec or issue to create sub-issue from"],
        }
    ...
    spec_title = tech_spec.get(
        "title", f"Technical Implementation for {issue.identifier}"
    )
    ...
    try:
        ...
        if sub_issue:
            ...
            return {
                ...
                "messages": state.get("messages", [])
                + [f"Created sub-issue {sub_issue.identifier} for technical review"],
            }
        else:
            return {
                "status": "failed",
        ...
        return {
            "status": "failed",
            "messages": state.get("messages", [])
            + [f"Error creating sub-issue: {str(e)}"],
        }
