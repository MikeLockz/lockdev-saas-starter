...
def implementation_engineer_node(state: AgentState) -> dict:
    ...
    if current_work:
        work_type = (
            ...
            else getattr(current_work, "type", "BACKEND")
        )
        mode = work_type
    else:
    ...
    prompt = prompt_template.format(contract=contract, task=task, context=context)
...
def implementation_engineer_correction_node(state: AgentState) -> dict:
    ...
    result = run_claude_code(
        prompt=prompt, working_dir=working_dir, allowed_tools=["Read", "Edit", "Bash"]
    )
