...
def get_device_string(user_agent: Optional[str]) -> str:
...
async def get_my_proxy_patients(
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    proxy_stmt = select(Proxy).where(Proxy.user_id == current_user.id).where(Proxy.deleted_at == None)
    ...
    stmt = (
        ...
        .where(
            (PatientProxyAssignment.expires_at == None) | 
            (PatientProxyAssignment.expires_at > now)
        )
    ...
    for assignment, patient in rows:
        patients.append(ProxyPatientRead(
