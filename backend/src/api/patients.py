...
async def list_patients(
    org_id: UUID,
    name: Optional[str] = Query(None, description="Fuzzy search by name"),
    mrn: Optional[str] = Query(None, description="Search by MRN"),
    status_filter: Optional[str] = Query(None, alias="status", description="ACTIVE or DISCHARGED"),
...
async def discharge_patient(
    ...
    enrollment.discharged_at = datetime.now(timezone.utc)
...
class ContactMethodUpdate(ContactMethodCreate):
    ...
    is_safe_for_voicemail: Optional[bool] = None
...
async def create_contact_method(
    ...
    if contact_data.is_primary:
        update_stmt = (
            ...
            .where(ContactMethod.is_primary == True)
        )
...
async def get_care_team(
    ...
    stmt = (
        ...
        .where(CareTeamAssignment.removed_at == None)
        .order_by(
...
async def assign_to_care_team(
    ...
    provider_stmt = (
        ...
        .where(Provider.deleted_at == None)
...
async def remove_from_care_team(
    ...
    if not assignment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Care team assignment not found"
        )
