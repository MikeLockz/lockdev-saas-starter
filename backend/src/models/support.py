...
from sqlalchemy import String, ForeignKey, Text, DateTime, Boolean, func
...
class SupportTicket(Base, UUIDMixin, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(String, nullable=False, default="OPEN", index=True)  # OPEN, IN_PROGRESS, RESOLVED, CLOSED
    assigned_to_id: Mapped[Optional[UUID]] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
...
class SupportMessage(Base, UUIDMixin):
    ...
    ticket_id: Mapped[UUID] = mapped_column(ForeignKey("support_tickets.id", ondelete="CASCADE"), nullable=False, index=True)
    sender_id: Mapped[UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    
    body: Mapped[str] = mapped_column(Text, nullable=False)
    is_internal: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False) # UUIDMixin doesn't have created_at, doing it manually or using TimestampMixin (but that adds updated_at too)
