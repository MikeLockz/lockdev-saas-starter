...
from app.core.audit_sql import (
    SQL_JSONB_CHANGE_KEY_NAME, SQL_GET_SETTING, SQL_CREATE_ACTIVITY,
    SQL_AUDIT_TABLE_MAIN, SQL_AUDIT_TABLE_WRAPPER,
    SQL_JSONB_SUBTRACT, SQL_CREATE_OPERATOR_MINUS
)
...
async def test_audit_log():
    ...
    if sa.event.contains(activity_table, 'after_create', versioning_manager.create_audit_table):
        sa.event.remove(activity_table, 'after_create', versioning_manager.create_audit_table)
    if sa.event.contains(activity_table, 'after_create', versioning_manager.create_operators):
        sa.event.remove(activity_table, 'after_create', versioning_manager.create_operators)
    ...
    async with engine.begin() as conn:
        ...
        # Operators
        await conn.execute(sa.text("DROP FUNCTION IF EXISTS jsonb_subtract(jsonb,jsonb) CASCADE"))
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Verify activity
        result = await session.execute(sa.text("SELECT * FROM activity WHERE table_name = 'article'"))
